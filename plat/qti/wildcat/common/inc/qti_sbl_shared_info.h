...
typedef struct _secboot_pbl_profile_marker {
	uint32_t secboot_pbl_authenticate_init_entry; /**< Start of
	...
	uint32_t secboot_pbl_verify_cert_chain; /**< Start of verify cert chain
						 */
	uint32_t secboot_pbl_verify_image_signature; /**< start of verify image
							signature */
	uint32_t secboot_pbl_authenticate_close; /**< secboot_authenticate is
						    complete */
...
typedef struct secboot_code_hash_info_type {
	unsigned long code_address; /**< Address (pointer value) of the code
				       that was hashed */
	uint32_t code_length;	    /**< the code length */
	uint32_t image_hash_length; /**< hash length - e.g 20 for SHA1, 32 for
				       SHA256 */
	uint8_t image_hash[SECBOOT_IMAGE_HASH_SIZE]; /**< hash of HEADER + CODE
						      */
} secboot_image_hash_info_type;

typedef struct secboot_verified_info_type {
	uint32_t version_id; /**< The version id (define the secboot lib
				version) */
	uint64_t sw_id; /**< The software id (upper 32 bits:version, lower 32
			   bits:type) the image was signed with */
	uint32_t enable_debug; /**< Value of the debug settings from the
				  attestation cert, i.e., SECBOOT_DEBUG_NOP,
				  SECBOOT_DEBUG_DISABLE, SECBOOT_DEBUG_ENABLE */
	secboot_image_hash_info_type image_hash_info; /**< Hash of the header +
							 code */

	uint32_t enable_root_key_control; /**< Value to indicate if the configu
					     fuse region is writeable or not.
					     i.e
					     SECBOOT_ROOT_KEY_CONTROL_ENABLE,
							  SECBOOT_ROOT_KEY_CONTROL_DISABLE
					   */
	secboot_metadata_type metadata;	  /** Metadata from the image that is
					     verified */

	secboot_pbl_profile_marker_type secboot_timestamps; /**< Performance
							       timestamps */
