...
/*
 * Log and memcpy-style helper implementations required to link the TME
 * message-layer sources (TmeMessage.c, tmeintf sources).  Their headers under
 * drivers/qti/tme/inc/common/ declare TFA_LOG_ERR() / TFA_LOG_ERR_FATAL()
 * (which expand to tfa_log_<n>()) and memscpy(), but only as declarations -
 * this file supplies the bodies.
 *
 * The tfa_log_<n>() family receives already-unpacked uintptr_t arguments and
 * no format string, so every value is printed as hex.
 *
 * Only the arities actually called are implemented - tfa_log_0 through
 * tfa_log_3 - plus memscpy().  The widest caller is TmeMessage.c's
 * TZBSP_TME_MESSAGE_ENCODE_MESSAGE_LENGTH_ERROR, which passes three arguments.
 * The header declares tfa_log_0..16 and memsmove(), but the unused ones are
 * deliberately omitted rather than carried here as dead forwarders.
 *
 * The log macro's arity dispatch (GET_MACRO_TFA in tzbsp_log.h) token-pastes
 * the argument count, so only the selected tfa_log_<n> is ever referenced.  A
 * new log call with four or more arguments will therefore fail to link until
 * the matching tfa_log_<n> is added below - likewise memsmove() if a caller
 * appears.
 */
